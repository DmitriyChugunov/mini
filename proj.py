...
def test_delete_expired_links(connection):
    try:
        ...
        with connection.cursor() as cursor:
            cursor.execute("DELETE FROM url_project WHERE expiration_date < %s", (current_date,))
...
def test_get_user_links(cursor, user_id):
    try:
        cursor.execute("SELECT original_url, short_url, expiration_date FROM url_project WHERE user_id = %s", (user_id,))
...
def test_get_expiration_date(cursor, short_url):
    try:
        cursor.execute("SELECT expiration_date FROM url_project WHERE short_url = %s", (short_url,))
        expiration_date = cursor.fetchone()
        return expiration_date[0] if expiration_date else None
    except Exception as ex:
        print(f"Ошибка при получении даты истечения срока действия сжатой ссылки: {ex}")
        return None
...
try:
    ...
    with connection.cursor() as cursor:
        ...
        user_type = input("Вы зарегистрированный пользователь? (да/нет): ").lower()
        ...
        if user_type == "да":
                ...
                if profiles == "да":
                    ...
                    if user_links:
                        ...
                        for link in user_links:
                            ...
                            print(f"Дата истечения срока действия: {expiration_date}")
                    else:
                    ...
                    people_url = input("Введите ссылку для сжатия:")  # переменная для проверки введенной ссылки

                    expiration_days = int(input("Введите количество дней для срока действия ссылки:"))  # Запросить срок действия в днях

                    s = pyshorteners.Shortener()

                    # Оригинальная URL ссылка

                    if people_url.startswith("http://") or people_url.startswith(
                        ...
                        # Получите идентификатор текущего пользователя из предыдущего входа в систему
                        ...
                        if user_id is not None:
                            ...
                            cursor.execute(
                                ...
                                (user_id, people_url, short_url, expiration_date))
                            ...
                            print("Невозможно добавить ссылку в профиль: неправильное имя пользователя или пароль.")
            ...
            user_type2 = input("Свои ссылки можно сжимать только после регистрации. Вы хотите зарегистрироваться?(да/нет)")
            if user_type2 == "да":
                ...
                print("Вы можете войти или зарегистрироваться для использования сервиса сжатия URL.")
            else:
                print("Незарегистрированным пользователям можно только посмотреть работу сжатия наших ссылок:")
                original_url2 = "https://auth.hr.alabuga.ru/auth/realms/Alabuga/protocol/openid-connect/auth?client_id=alb-player&redirect_uri=https%3A%2F%2Fhr.alabuga.ru%2F&state=850b5984-7872-4179-bcfd-abad20025de3&response"
                s = pyshorteners.Shortener()
                short_url2 = s.tinyurl.short(original_url2)  # переменная = сжатие оригинальной ссылки
